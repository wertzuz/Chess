...
func convert_time_in_ms_to_string(time_remaining int64) string { //format the time portable string that is exactly 5 chars long
	var minutes = time_remaining / 60000
	var seconds = time_remaining % 60000
	var milliseconds = seconds % 1000
	seconds = seconds / 1000

	var str_minutes = "0" + strconv.Itoa(int(minutes))
	var str_seconds = "0" + strconv.Itoa(int(seconds))
	var str_milliseconds = "00" + strconv.Itoa(int(milliseconds))
