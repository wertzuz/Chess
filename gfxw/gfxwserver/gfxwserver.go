...
var anschlagzeit = 0.002   // Standard auf 2 ms gesetzt
var abschwellzeit = 0.750  // Standard auf 750 ms gesetzt
var haltepegel float64 = 0 // Standard auf 0 % gesetzt ; 1 = 100 %
var ausklingzeit = 0.006   // Standard auf 6 ms gesetzt
var pulsweite = 0.375      // nur wichtig für Rechteck-Signale, hier: Prozentsatz HIGH (Pulsweite)

var tempo uint8 = 120                                               // "Schläge" = Viertelnoten pro Minute"
var tVollnote = 4 * uint16((60*1000+uint32(tempo)/2)/uint32(tempo)) // Zeit einer vollen Note in ms
...
var serverLaeuft bool

// Es gibt 4 Tastenbelegungen: Standard, SHIFT, ALT GR, ALT GR mit SHIFT.

var z1 = [4]string{",-.", ";_:", "·–…", "×—÷"}
var z2 = [4]string{"0123456789", "=!\"§$%&/()", "}¹²³¼½¬{[]", "°¡⅛£¤⅜⅝⅞™±"}
var z3 = [4]string{"abcdefghijklmnopqrstuvwxyz", "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "æ“¢ð€đŋħ→̣ĸłµ”øþ@¶ſŧ↓„ł«»←", "Æ‘©Ð€ªŊĦı˙&Łº’ØÞΩ®ẞŦ↑‚Ł‹›¥"}
var z4 = [4]string{",/*-+", ",/*-+", ",/*-+", ",/*-+"}                     //NUM-Block
var z5 = [4]string{"0123456789", "0123456789", "0123456789", "0123456789"} //NUM-BLOCK
...
// GibNotenTempo Erg.: Das aktuelle Tempo ist geliefert, d.h. die Anzahl der Viertelnoten pro Minute.
func GibNotenTempo() uint8 {
	return tempo
}

// SetzeNotenTempo Vor.: 30 <= tempo <= 240
...
func ton(tges uint16, f float64) []byte {
	var laenge = r * uint32(tges) * uint32(b*k) / 1000
	var dateigroesseMinus8 = laenge + 44 - 8
	var bytes = make([]byte, 44+laenge)
	var w float64

	// "Dateikopf gemäß RIFF-WAVE-Format
	copy(bytes, "RIFF")
	copy(bytes[4:], uint32toSlice(dateigroesseMinus8)) //DATEIGRÖSSE - 8 ----------------------------------------------------
...
func SpieleNote(tonname string, laenge float64, wartedauer float64) {
	var o = byte(tonname[0]) - 48
	var freq = frequenzen[tonname[1:]]
	lock()
	for i := uint8(7); i > o; i-- {
		freq = freq / 2
	}
...
func starteGfxServer(quellIP string, portnummer uint16, f func(string) string) {
	...
	connMaus.Close()
	connTast.Close()
	serverLaeuft = false // Hiermit wird das Hauptprogramm 'main' und damit auch der Server beendet!
}
...
func bearbeiteAnfrage(conn net.Conn, f func(string) string) {
	var l = make([]byte, 4)    //Byte-Slice für die Länge der Nachricht
	...
	if serverprotokoll {
		...
		fmt.Println(string('"') + antwort + string('"'))
		fmt.Println("=======================================================")
	}
}
